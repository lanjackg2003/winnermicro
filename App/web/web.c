...
Who         When          What
--------    ----------    ----------------------------------------------
wangyf     2011.01.21  Create the file  
-------------------------------------------------------------------------*/
#include <assert.h>
#include <stdlib.h>
#include <stdio.h>
...
extern const INT8U SysCreatedDate[];
extern const INT8U SysCreatedTime[];
...
#endif
char * Encry_Open[]={"Disable", "WEP64", "WEP128", "WPA-PSK(TKIP)", "WPA-PSK(CCMP)", "WPA2-PSK(TKIP)", "WPA2-PSK(CCMP)"};
...
unsigned int Web_Baud[BAUDRATE_BELOW]= {115200, 57600, 38400, 19200, 9600, 4800, 2400, 1200};
...
static INT strtodec(INT *dec, char *str)
{
	INT i = -1;
	INT dd = 0, sign = 1;
...
INT16U  Web_parse_line(char * id_char,INT16U * after_id_len,char * idvalue,INT16U * Value_Offset,INT8U * Id_type)
{
	char idbuffer[MAX_ID_BUFFER_LEN];
	INT8U j=0, mode, encrypt;
	ID_VALUE  *idtble;
	INT8U tableid=0;
...
//	DBGPRINT("###kevin debug Web_parse_line = %s\n\r",idbuffer);
	...
	case Web_Key_Id:
		...
		tls_param_get(TLS_PARAM_ID_ENCRY, (void *)&encrypt, FALSE);
		if(mode == IEEE80211_MODE_INFRA || encrypt != ENCRYPT_TYPE_OPEN_SYS)
		{
...
char * do_cgi_config(int iIndex, int iNumParams, char *pcParam[], char *pcValue[],INT8U *  NeedRestart)
{
	int i;
	INT Value;
...
char * do_cgi_advance(int iIndex, int iNumParams, char *pcParam[], char *pcValue[], INT8U * NeedRestart)
{
	int i;
	INT Value;
	INT32U HexValue;
	...
	for (i = 0; i < iNumParams - 1; i++)		//not care about "Save"
	{
		...
		else if (strcmp(pcParam[i], "EscP") == 0)
		{
			...
			if(100 <= EscapePeriod <= 10000){
				EscapePeriod = (EscapePeriod/100)*100; /* drop remainder */
...
char * do_cgi_firmware(int iIndex, int iNumParams, char *pcParam[], char *pcValue[], INT8U * NeedRestart)
{
	int i;
...
char * do_cgi_webindex(int iIndex, int iNumParams, char *pcParam[], char *pcValue[], INT8U * NeedRestart)
{
		int i;
		INT Value;
		//int KeyLen = 0;	
