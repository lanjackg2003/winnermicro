...
#if TLS_CONFIG_APSTA
static int ricmd_ssid2_proc(char *buf, u32 length, 
        char *cmdrsp_buf, u32 *cmdrsp_size)
{
    struct tls_hostif_cmdrsp *cmdrsp = (struct tls_hostif_cmdrsp *)cmdrsp_buf;
    struct tls_hostif_cmd *cmd;
    ...
    if (cmd->cmd_hdr.ext & 0x1) {
        ...
        if (cmd->params.ssid.ssid_len > 32 || cmd->params.ssid.ssid_len == 0)
            ...
            cmd_ssid.ssid_len = cmd->params.ssid.ssid_len;
            MEMCPY(cmd_ssid.ssid, cmd->params.ssid.ssid, cmd_ssid.ssid_len);
            err = tls_cmd_set_ssid2(&cmd_ssid, update_flash);
            if (err)
                err = CMD_ERR_INV_PARAMS;
        }

        tls_hostif_fill_cmdrsp_hdr(cmdrsp, HOSTIF_CMD_SSID2, err, 0);
        *cmdrsp_size = sizeof(struct tls_hostif_cmd_hdr);
    } else {
        /*  get ssid */
        if (length != sizeof(struct tls_hostif_cmd_hdr)) {
        ...
        if (!err) {
            tls_cmd_get_ssid2(&cmd_ssid);
            cmdrsp->params.ssid.ssid_len = cmd_ssid.ssid_len;
            MEMCPY(cmdrsp->params.ssid.ssid, cmd_ssid.ssid, cmd_ssid.ssid_len);
            tls_hostif_fill_cmdrsp_hdr(cmdrsp, HOSTIF_CMD_SSID2, 0, 1);
            *cmdrsp_size = sizeof(struct tls_hostif_cmd_hdr) + 1 + cmd_ssid.ssid_len;
        } else {
            tls_hostif_fill_cmdrsp_hdr(cmdrsp, HOSTIF_CMD_SSID2, err, 0);
            *cmdrsp_size = sizeof(struct tls_hostif_cmd_hdr); 
...
int ricmd_get_mac2_proc(char *buf, u32 length, 
        char *cmdrsp_buf, u32 *cmdrsp_size)
{
    struct tls_hostif_cmdrsp *cmdrsp = (struct tls_hostif_cmdrsp *)cmdrsp_buf;
    u8 *mac_addr;
    ...
    do {
        ...
        mac_addr = (u8* )wpa_supplicant_get_mac2();
        if (!mac_addr) {
            err = CMD_ERR_OPS; 
        } else {
            MEMCPY(&cmdrsp->params.mac.addr, mac_addr, ETH_ALEN);
    ...
    if (err) {
        tls_hostif_fill_cmdrsp_hdr(cmdrsp, HOSTIF_CMD_MAC2, err, 0);
        *cmdrsp_size = sizeof(struct tls_hostif_cmd_hdr);
    }
    else {
        tls_hostif_fill_cmdrsp_hdr(cmdrsp, HOSTIF_CMD_MAC2, 0, 1);
        *cmdrsp_size = sizeof(struct tls_hostif_cmd_hdr) + ETH_ALEN;
    } 
...
int ricmd_link2_status_proc(char *buf, u32 length, 
        char *cmdrsp_buf, u32 *cmdrsp_size)
{
    struct tls_hostif_cmdrsp *cmdrsp = (struct tls_hostif_cmdrsp *)cmdrsp_buf;
    struct tls_cmd_link_status_t lks;
    ...
    tls_hostif_fill_cmdrsp_hdr(cmdrsp, HOSTIF_CMD_LINK2_STATUS, err, ext);
...
int ricmd_atrm_proc(char *buf, u32 length, char *cmdrsp_buf, u32 *cmdrsp_size)
{
	struct tls_hostif_cmdrsp *cmdrsp = (struct tls_hostif_cmdrsp *)cmdrsp_buf;
	struct tls_hostif_cmd *cmd = (struct tls_hostif_cmd *)buf;
	int err = CMD_ERR_OK;
	...
	if (!err) 
	{
		...
		{ 
			do 
			{
				...
				if (length > 1) 
				{
					if (cmd->params.atrm.proto > 1) 
					{
				//		printf("kevin debug skt.proto = %d\r\n", cmd->params.atrm.proto);
						break;
					}
				...
				if (length > 1) 
				{
					...
					{
				//		printf("kevin debug skt.server = %d\r\n", cmd->params.atrm.server);
						break;
					}
			...
			//		printf("kevin debug err 3\r\n");
				...
				if (length > skt.host_len) 
				{
					...
					/* TCP SERVER  */
						skt.timeout = get_unaligned_be32(p); 
						//skt.host_len = 4;
						sprintf(skt.host_name, "%d", skt.timeout); 
		...
		//			printf("kevin debug err 4\r\n");
				...
				if (length == 2)
				{
					...
					skt.port = get_unaligned_be16(p);
			//		printf("kevin debug skt.port %x\r\n", skt.port);
				} 
				else
				{
		//			printf("kevin debug err 5\r\n");
					break;
				}
				err = CMD_ERR_OK;
			} while(0);
		...
		/* get default socket info */
			...
			if ((strlen(skt.ip_addr) == 4) && (skt.client)) 
			{
				skt.host_len = 4;
				MEMCPY(p, skt.ip_addr, 4);
			} 
			else if (skt.client) 
			{
				skt.host_len = strlen(skt.host_name);
	...
	//		printf("kevin debug *cmdrsp_size = %d\r\n", *cmdrsp_size);
		}
	} 
	else 
	{
...
int ricmd_flsr_proc(char *buf, u32 length, char *cmdrsp_buf, u32 *cmdrsp_size)
{
	struct tls_hostif_cmdrsp *cmdrsp = (struct tls_hostif_cmdrsp *)cmdrsp_buf;
	struct tls_hostif_cmd *cmd;
	u8 buff[32];
	...
	if (cmd->cmd_hdr.ext & 0x1) 
	{
		if (cmd->params.flsr.length  > 32 || cmd->params.flsr.length == 0)
		{
			tls_hostif_fill_cmdrsp_hdr(cmdrsp, HOSTIF_CMD_FLSR, CMD_ERR_INV_PARAMS, 0);
			*cmdrsp_size = sizeof(struct tls_hostif_cmd_hdr);
		}
		else 
		{
		//	printf("kevin debug read = %x, %d\r\n", cmd->params.flsr.reg_base_addr, cmd->params.flsr.length);
			tls_fls_read(cmd->params.flsr.reg_base_addr, buff, cmd->params.flsr.length);
		//	printf("kevin debug read data = %x, %x, %x, %x\r\n", buff[0], buff[1], buff[2], buff[3]);
			MEMCPY(&cmdrsp->params.flsr.value, buff, cmd->params.flsr.length);
			tls_hostif_fill_cmdrsp_hdr(cmdrsp, HOSTIF_CMD_FLSR, 0, 1);
			*cmdrsp_size = sizeof(struct tls_hostif_cmd_hdr) + 1 + cmd->params.flsr.length;
		}
	} 
	else 
	{
...
int ricmd_flsw_proc(char *buf, u32 length, char *cmdrsp_buf, u32 *cmdrsp_size)
{
	struct tls_hostif_cmdrsp *cmdrsp = (struct tls_hostif_cmdrsp *)cmdrsp_buf;
	struct tls_hostif_cmd *cmd;
	...
	if (cmd->cmd_hdr.ext & 0x1) 
	{
		if (cmd->params.flsw.length  > 32 || cmd->params.flsw.length == 0)
		{
			tls_hostif_fill_cmdrsp_hdr(cmdrsp, HOSTIF_CMD_FLSW, CMD_ERR_INV_PARAMS, 0);
			*cmdrsp_size = sizeof(struct tls_hostif_cmd_hdr);
		}
		else 
		{
		//	printf("kevin debug write = %x, %d\r\n", cmd->params.flsw.reg_base_addr, cmd->params.flsw.length);
			tls_fls_write(cmd->params.flsw.reg_base_addr, cmd->params.flsw.value, cmd->params.flsw.length);
			tls_hostif_fill_cmdrsp_hdr(cmdrsp, HOSTIF_CMD_FLSW, CMD_ERR_OK, 0);
			*cmdrsp_size = sizeof(struct tls_hostif_cmd_hdr);
		}
	} 
	else 
...
#if TLS_CONFIG_APSTA
{HOSTIF_CMD_SSID2,        ricmd_ssid2_proc        },
{HOSTIF_CMD_MAC2,         ricmd_get_mac2_proc     },
{HOSTIF_CMD_LINK2_STATUS, ricmd_link2_status_proc },
#endif
