...
static const char at_resp_rpt[] = "+SKTRPT=";
#endif
...
static void uart_set_uart1_mode(u32 cmd_mode)
{
	...
	{
		tls_uart_set_fc_status(uart_st[1].uart_port->uart_no, TLS_UART_FLOW_CTRL_HARDWARE);
	}
	else
	{
		tls_uart_set_fc_status(uart_st[1].uart_port->uart_no, TLS_UART_FLOW_CTRL_NONE);
	}
}
...
static void uart_set_uart0_mode(u32 cmd_mode)
{
	...
	if(UART_TRANS_MODE == cmd_mode)
	{
		tls_uart_set_fc_status(uart_st[0].uart_port->uart_no, TLS_UART_FLOW_CTRL_HARDWARE);
	}
	else
	{
		tls_uart_set_fc_status(uart_st[0].uart_port->uart_no, TLS_UART_FLOW_CTRL_NONE);
	}
}
...
void tls_uart_init(void)
{
    ...
    /* setting uart0 */
    if(WM_SUCCESS  != tls_uart_port_init(TLS_UART_0, NULL))
		return;
	...
	if (uart_cfg.baudrate){
		uart1_delaytime = (UART_NET_SEND_DATA_SIZE * (10*1000000/uart_cfg.baudrate)/1000 +5);
	}else{
		uart1_delaytime = 100;
	}
#endif
    ...
    if (hif->hostif_mode == HOSTIF_MODE_UART1_HS) {
		...
		if(WM_SUCCESS != tls_uart_port_init(TLS_UART_1, &uart_opts))
			return;
        ...
        if (tls_cmd_get_auto_mode())
            ...
            uart->cmd_mode = UART_TRANS_MODE;
						tls_uart_set_fc_status(uart->uart_port->uart_no,TLS_UART_FLOW_CTRL_HARDWARE);
        }
        else
#endif //TLS_CONFIG_SOCKET_RAW
				{
            uart->cmd_mode = UART_ATCMD_MODE; 	//指令模式关闭流控
						tls_uart_set_fc_status(uart->uart_port->uart_no,TLS_UART_FLOW_CTRL_NONE);
				}
		}else {
        ;
    }
...
static s16 tls_uart0_task_rx_cb(char *buf, u16 len)
{
	...
	if((UART_TRANS_MODE == uart->cmd_mode) && (3 == uart->uart_port->plus_char_cnt))
	{
		uart->cmd_mode = UART_ATCMD_MODE;
		tls_uart_set_fc_status(uart->uart_port->uart_no,TLS_UART_FLOW_CTRL_NONE);
	}
...
static s16 tls_uart0_task_tx_cb(struct tls_uart_port *port)
{
	struct tls_uart *uart = &uart_st[0];
	...
	if(uart->tx_mailbox)
		tls_os_mailbox_send(uart->tx_mailbox, (void *)MBOX_MSG_UART_TX);	//可能有问题
        ...
        TLS_UART_MODE_T uart_mode)
{
	...
	tls_uart_rx_register(uart_no, tls_uart0_task_rx_cb);
	tls_uart_tx_register(uart_no, tls_uart0_task_tx_cb);
...
#endif    
    {
        ...
        if (uart_no == TLS_UART_0) {
            ...
            tls_os_task_create(NULL, NULL,
                    ...
                    TLS_UART1_RX_TASK_PRIO,
                    0);
        }

		tls_timer2_irq_register(uart);
...
static void modify_atcmd_tail(struct tls_uart_circ_buf *recv, u8 *p)
{
	...
	if(cmd_len > 128)
    {
        recv->tail = recv->head;
		p = NULL;
		TLS_DBGPRT_INFO("EOF char find > 128 \r\n");
    }
	else
	{
...
//	TLS_DBGPRT_INFO("A1 %d, %d\r\n", recv->tail, recv->head);
    while((CIRC_CNT(recv->head, recv->tail, TLS_UART_RX_BUF_SIZE) >= 3) && (atcmd_start == NULL)) 
	{ // check "at+" char
			...
			(recv->buf[(recv->tail + 2) & (TLS_UART_RX_BUF_SIZE - 1)] == '+')) 
		{
         ...
         //   TLS_DBGPRT_INFO("ptr_eol = 0x%x\n", ptr_eol);
            if (!ptr_eol)
            {// 没有结束符，可能只收到半个命令
            	if(CIRC_CNT(recv->head, recv->tail, TLS_UART_RX_BUF_SIZE) > 128)
            	{
            		recv->tail = recv->head;
            	}
				else
		...
		{	//start of string is not "at+", and string not include '\r' and '\n' eat the string
            ptr_eol = parse_atcmd_eol(uart);
            if (!ptr_eol)
            {
                recv->tail = recv->head;
...
#if TLS_CONFIG_SOCKET_RAW
void uart_net_send(struct tls_uart *uart, 
        u32 head, u32 tail, int count)
{
    struct tls_uart_circ_buf *recv = &uart->uart_port->recv;
    int buflen;
	int bufcopylen = 0;
...
RESENDBUF:
	...
	if ((tail + buflen) > TLS_UART_RX_BUF_SIZE){
		...
		MEMCPY(uart_net_send_data, recv->buf + tail, bufcopylen);
		MEMCPY(uart_net_send_data+bufcopylen, recv->buf, buflen - bufcopylen);
	}else{
    ...
    if (def_socket) {
	    ...
	    do{
		 ...
		 //	printf("\nsocket err val\n");
			...
			tls_timer2_start(uart1_delaytime);
			return;
		 }
	        if(err == ERR_MEM)
	        {
	        	if(TLS_UART_FLOW_CTRL_HARDWARE == uart->uart_port->fcStatus && TLS_UART_FLOW_CTRL_HARDWARE == uart->uart_port->opts.flow_ctrl)
	        	{
...
{
    struct tls_uart_circ_buf * precvmit = tls_hostif_get_recvmit(tx_msg->u.msg_tcp.sock);
    struct tls_hostif *hif = tls_get_hostif();
    struct pbuf      *p; 
    u16              buflen;
    u16              copylen;
...
TLS_DBGPRT_INFO("precvmit->head=%d, precvmit->tail=%d\n", precvmit->head, precvmit->tail);
	...
	if(copylen == 0)
	{
        ...
        if (buflen < copylen)
            copylen = buflen;
        pbuf_copy_partial(p, 
                precvmit->buf + precvmit->head,
                copylen, tx_msg->offset);
        precvmit->head = (precvmit->head + copylen) & (TLS_SOCKET_RECV_BUF_SIZE - 1);
TLS_DBGPRT_INFO("precvmit->head=%d, precvmit->tail=%d\n", precvmit->head, precvmit->tail);
        ...
        struct tls_hostif_tx_msg *tx_msg)
{
    ...
    while (1) {
        ...
        copylen = pbuf_copy_partial(p, 
                ...
                copylen, tx_msg->offset);
        port->xmit.head = (port->xmit.head + copylen) & (TLS_UART_TX_BUF_SIZE - 1);
        tx_msg->offset += copylen;
        if (tx_msg->offset >= p->tot_len)
            break;
    }; 
    ...
    if (!(port->inputstate & INS_SYNC_CHAR)) {
        ...
        if (recv->buf[recv->head] == RICMD_SYNC_FLAG) {
            ...
            if (numbytes < 8) {
                ...
                TLS_DBGPRT_INFO("recevied crc value = 0x%x, check crc value = %x\n",
                        orgi_crc8, new_crc8);
#endif
...
//		TLS_DBGPRT_INFO("%d, %d\r\n", pfwup->number, pfwup->sum);
		if(session_id)
		{
			if(get_crc32((u8 *)pfwup, UART_UPFW_DATA_SIZE - 12) == pfwup->crc32)
			{
				if(TLS_FWUP_STATUS_OK == tls_fwup_set_update_numer(pfwup->number))
				{
					status = 1;
					uart_fwup_rsp(uart->uart_port->uart_no, status);
					tls_fwup_request_sync(session_id, (u8 *)pfwup, UART_UPFW_DATA_SIZE);
				}
				else
				{
...
//TLS_DBGPRT_INFO("port->cmd_mode=%d\r\n", port->cmd_mode);
    ...
    if(uart->cmd_mode == UART_TRANS_MODE) 
	{
        ...
        if (uart->rx_idle && data_cnt) {
			tcptimedelayflag = 0;
            send_data = 1;
        } else if (data_cnt >= UART_NET_SEND_DATA_SIZE) {
	        tcptimedelayflag = 0;
            send_data = 1; 
			tls_timer2_stop();
        } else {
        	if ((tcptimedelayflag == 0)&&(data_cnt)){
   	    		tls_timer2_start(uart1_delaytime);
				tcptimedelayflag = 1;
        	}
            /* do nothing */
	...
	else if(UART_ATSND_MODE == uart->cmd_mode)
	{
		...
		if (uart->rx_idle && data_cnt) {
			send_data = 1;
		} else if (data_cnt >= uart->sksnd_cnt) {
			send_data = 1; 
		} else {
			/* do nothing */
			return;
...
#if TLS_CONFIG_SOCKET_RAW
static int str_to_uint(char *buf,  u8 len, u32 *d)
{
    int i;
...
#if TLS_CONFIG_SOCKET_RAW
TLS_DBGPRT_INFO("tx_msg->u.msg_event.buflen=%d\r\n", tx_msg->u.msg_event.buflen);
		if(0 == memcmp(at_resp_rpt,tx_msg->u.msg_event.buf + tx_msg->offset, 8))
		{
			for(i=tx_msg->offset;i<tx_msg->u.msg_event.buflen;i++)
			{
				if(tx_msg->u.msg_event.buf[i] == ',')
					break;
			}

			if (i == tx_msg->u.msg_event.buflen)
				goto err;

			ret = str_to_uint((tx_msg->u.msg_event.buf + tx_msg->offset+8),(i-8), &socket);
			if(ret)
				goto err;
			TLS_DBGPRT_INFO("socket:%d\n", socket);

			for(j = i+1;j<tx_msg->u.msg_event.buflen;j++)
			{
				if(tx_msg->u.msg_event.buf[j] == ',')
					break;
			}
			if (j == tx_msg->u.msg_event.buflen)
				goto err;
			
			ret = str_to_uint((tx_msg->u.msg_event.buf + tx_msg->offset + i+1), (j-i-1), &maxsize);
			if (ret)
				goto err;

			if(socket > 20)
				goto err;
			TLS_DBGPRT_INFO("socket = %d, maxsize=%d\r\n",socket, maxsize);	
			...
			if(precvmit)
			{
				...
				goto err;
			/*+SKTRPT=sockenum,datalen,ipaddr,port*/
			tls_uart_write(uart->uart_port, tx_msg->u.msg_event.buf + tx_msg->offset, tx_msg->u.msg_event.buflen);
			/*Data*/
			while(1)
			{
				...
				tls_uart_write(uart->uart_port,(char *)(precvmit->buf+precvmit->tail),ret);
			...
			tx_msg->offset = tx_msg->u.msg_event.buflen;

		}else if(0 != memcmp(at_resp_ok,tx_msg->u.msg_event.buf + tx_msg->offset,3) 
				&& 0 != memcmp(at_resp_err,tx_msg->u.msg_event.buf + tx_msg->offset,4))
		{
			for(i=tx_msg->offset;i<tx_msg->u.msg_event.buflen;i++)
			{
				if(tx_msg->u.msg_event.buf[i] == ',')
			...
			if(i == tx_msg->u.msg_event.buflen)
				goto err;
			ret = str_to_uint(tx_msg->u.msg_event.buf + tx_msg->offset,i, &socket);
			if(ret)
				goto err;
			ret = str_to_uint(tx_msg->u.msg_event.buf + tx_msg->offset + i + 1,tx_msg->u.msg_event.buflen-i-5, &maxsize);
			if(ret)
				goto err;
			if(socket > 20)
				goto err;
...
//TLS_DBGPRT_INFO("precvmit=0x%x\r\n",precvmit);	
			if(precvmit)
			{
				...
				goto err;
//TLS_DBGPRT_INFO("socket = %d, maxsize=%d\r\n",socket, maxsize);	
			tls_uart_write(uart->uart_port, (char *)at_resp_ok, 4);
			sprintf(result, "%d\r\n\r\n", maxsize);
			tls_uart_write(uart->uart_port,result, strlen(result));
...
#endif //TLS_CONFIG_SOCKET_RAW
		{
//TLS_DBGPRT_INFO("tx_msg->u.msg_event.buf + tx_msg->offset = %s, size=%d\r\n",tx_msg->u.msg_event.buf + tx_msg->offset, strlen(tx_msg->u.msg_event.buf + tx_msg->offset));	
                /* copy data to uart tx ring buffer */
                write_cnt = tls_uart_write(uart->uart_port, 
                        tx_msg->u.msg_event.buf + tx_msg->offset, 
                        tx_msg->u.msg_event.buflen);
