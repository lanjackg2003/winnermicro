...
*/
    ...
    if ((ps->ps_type == 1)||(ps->ps_type == 2) && (ps->wake_type == 1)) {
        if (((ps->delay_time < 100) || (ps->delay_time > 1000)) ||
            (ps->wake_time < 1000)) {
            return CMD_ERR_INV_PARAMS; 
        }
    }
...
static int tls_cmd_create_apsta_net(void)
{
	...
	memset(&ssid2, 0, sizeof(struct tls_cmd_ssid_t));
	
	tls_cmd_get_bssid(&bssid);
	tls_cmd_get_key(&key);
	tls_cmd_get_ssid2(&ssid2);

	if(bssid.enable)
	{
	    if (0 == ssid2.ssid_len)
	    {
            ret = tls_wifi_apsta_start_by_bssid(bssid.bssid, key.key, key.key_len, NULL, 0);
	    }
	    else
	    {
		    ret = tls_wifi_apsta_start_by_bssid(bssid.bssid, key.key, key.key_len, ssid2.ssid, ssid2.ssid_len);
	    }
	}
	else
	{
        ...
        if (0 == ssid2.ssid_len)
	    {
	        ret = tls_wifi_apsta_start(ssid.ssid, ssid.ssid_len, key.key, key.key_len, NULL, 0);
	    }
	    else
	    {
            ret = tls_wifi_apsta_start(ssid.ssid, ssid.ssid_len, key.key, key.key_len, ssid2.ssid, ssid2.ssid_len);
        }
	}
...
int tls_cmd_create_net( void )
{
	...
	tls_cmd_get_encrypt( &apinfo->encrypt);
	
	tls_cmd_get_channel( &apinfo->channel, &channel_en);

	tls_cmd_get_key((struct tls_cmd_key_t *)(&apinfo->keyinfo));
        ...
        struct tls_cmd_connect_t *conn)
{
    ...
    switch (wifi_mode) {
        case IEEE80211_MODE_INFRA:
            return tls_cmd_join_net();
#if TLS_CONFIG_APSTA
		case IEEE80211_MODE_APSTA:
		    return tls_cmd_create_apsta_net();
#endif
        case IEEE80211_MODE_IBSS:			
			return tls_cmd_create_ibss_net();
        case IEEE80211_MODE_AP:
            return tls_cmd_create_net();
...
int tls_cmd_disconnect_network(void)
{
	struct tls_hostif *hif = tls_get_hostif();
...
#if TLS_CONFIG_APSTA
int tls_cmd_set_ssid2(struct tls_cmd_ssid_t *ssid, u8 update_flash)
{
    struct tls_param_ssid params_ssid;
    ...
    params_ssid.ssid_len = ssid->ssid_len;
    MEMCPY(&params_ssid.ssid, ssid->ssid, ssid->ssid_len);
    tls_param_set(TLS_PARAM_ID_APSTA_SSID, (void *)&params_ssid, (bool)update_flash);

    memset(&apsta_psk, 0, sizeof(apsta_psk));
    tls_param_set(TLS_PARAM_ID_APSTA_PSK, (void *)&apsta_psk, (bool)update_flash);
...
{
	tls_wifi_set_oneshot_flag(oneshotflag);
