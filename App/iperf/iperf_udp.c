...
iperf_udp_recv(struct iperf_stream *sp)
{
	...
	IPF_DBG("bytes_received_this_interval = %d\n", sp->result->bytes_received_this_interval);
...
//    sp->jitter += (d - sp->jitter) / 16.0; //for double jitter
	sp->jitter += (d - sp->jitter) / 16;
