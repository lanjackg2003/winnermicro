...
package_parameters(struct iperf_test *test)
{
    ...
    *pstring = ' ';
	...
	printf("optbuf=%s, pstring=%s\n", optbuf, pstring);
#endif	
    os_snprintf(optbuf, sizeof(optbuf), "-P %d ", test->num_streams);
    strncat(pstring, optbuf, sizeof(optbuf));
    ...
    if (test->settings->socket_bufsize) {
        os_snprintf(optbuf, sizeof(optbuf), "-w %d ", test->settings->socket_bufsize);
        strncat(pstring, optbuf, sizeof(optbuf));
    }

    if (test->settings->rate) {
        os_snprintf(optbuf, sizeof(optbuf), "-b %llu ", test->settings->rate);
        //os_snprintf(optbuf, sizeof(optbuf), "-b %d ", test->settings->rate);
        strncat(pstring, optbuf, sizeof(optbuf));
    }

    if (test->settings->mss) {
        os_snprintf(optbuf, sizeof(optbuf), "-m %d ", test->settings->mss);
        strncat(pstring, optbuf, sizeof(optbuf));
    }

    if (test->no_delay) {
        os_snprintf(optbuf, sizeof(optbuf), "-N ");
        strncat(pstring, optbuf, sizeof(optbuf));
    }

    if (test->settings->bytes) {
        os_snprintf(optbuf, sizeof(optbuf), "-n %llu ", test->settings->bytes);
        //os_snprintf(optbuf, sizeof(optbuf), "-n %d ", test->settings->bytes);
        strncat(pstring, optbuf, sizeof(optbuf));
    }

    if (test->duration) {
        os_snprintf(optbuf, sizeof(optbuf), "-t %d ", test->duration);
        strncat(pstring, optbuf, sizeof(optbuf));
    }

    if (test->settings->blksize) {
        os_snprintf(optbuf, sizeof(optbuf), "-l %d ", test->settings->blksize);
        strncat(pstring, optbuf, sizeof(optbuf));
    }

    if (test->settings->tos) {
        os_snprintf(optbuf, sizeof(optbuf), "-S %d ", test->settings->tos);
        strncat(pstring, optbuf, sizeof(optbuf));
    }
        ...
        if (bytes_sent > 0) {
            ...
            if (test->protocol->id == Ptcp) {
                ...
                printf(report_bw_jitter_loss_format, sp->socket, start_time,
                        ...
                        sp->packet_count, (100 * sp->cnt_error / sp->packet_count));
			#endif
                if (test->role == 'c') {
    ...
    if (test->num_streams > 1) {
        ...
        if (test->protocol->id == Ptcp) {
            ...
            printf(report_sum_bw_jitter_loss_format, start_time, end_time, ubuf, nbuf, avg_jitter,
                ...
                lost_packets, total_packets,  (100 * lost_packets / total_packets));
        }
    }
