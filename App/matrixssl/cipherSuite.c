...
{
	int32	err;
	psAesCipherContext_t *ctx;
	
	if (type == INIT_ENCRYPT_CIPHER) {	
		memset(&(sec->encryptCtx), 0, sizeof(psAesCipher_t));	
		ctx = (psAesCipherContext_t*)&sec->encryptCtx;
		if ((err = psAesInit(ctx, sec->writeIV, sec->writeKey,
							 keysize)) < 0) {
			return err;
		}
	} else { /* Init for decrypt */	
		memset(&(sec->decryptCtx), 0, sizeof(psAesCipher_t));	
		ctx = (psAesCipherContext_t*)&sec->decryptCtx;
		if ((err = psAesInit(ctx, sec->readIV, sec->readKey,
							 keysize)) < 0) {
			return err;
		}
	}
...
{
	ssl_t	*lssl = ssl;
	psAesCipherContext_t *ctx = (psAesCipherContext_t*)&lssl->sec.encryptCtx;
...
{
	ssl_t	*lssl = ssl;
	psAesCipherContext_t *ctx = (psAesCipherContext_t*)&lssl->sec.decryptCtx;
