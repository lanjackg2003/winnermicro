						...
						unsigned char *mac)
{
	...
	if(ssl->rec.type != SSL_RECORD_TYPE_APPLICATION_DATA || mode == HMAC_CREATE || ssl->deBlockSize > 1 || (data != NULL && len > 0)){
#endif
	psSha1Update(psha, data, len);
#ifdef USE_APP_DATA_PARTIAL_PARSING
	}
	if(ssl->rec.type != SSL_RECORD_TYPE_APPLICATION_DATA || mode == HMAC_CREATE ||ssl->deBlockSize > 1 || (data == NULL && len == 0) )
#endif
	{
		psSha1Final(psha, ihash);

		psSha1Init(psha);
		psSha1Update(psha, key, SHA1_HASH_SIZE);
		psSha1Update(psha, pad2, 40);
		psSha1Update(psha, ihash, SHA1_HASH_SIZE);
		psSha1Final(psha, mac);
...
*/
int32 ssl3HMACMd5(ssl_t *ssl, int32 mode, unsigned char *key, unsigned char *seq, 
						unsigned char type, unsigned char *data, uint32 len,
						unsigned char *mac)
{
...
#endif
		psMd5Init(pmd5);
		psMd5Update(pmd5, key, MD5_HASH_SIZE);
		psMd5Update(pmd5, pad1, 48);
		psMd5Update(pmd5, seq, 8);
		ihash[0] = type;
		ihash[1] = (len & 0xFF00) >> 8;
		ihash[2] = len & 0xFF;
		psMd5Update(pmd5, ihash, 3);
#ifdef USE_APP_DATA_PARTIAL_PARSING
	}
	if(ssl->rec.type != SSL_RECORD_TYPE_APPLICATION_DATA || mode == HMAC_CREATE || ssl->deBlockSize > 1 || (data != NULL && len > 0)){
#endif
		psMd5Update(pmd5, data, len);
#ifdef USE_APP_DATA_PARTIAL_PARSING
	}
	if(ssl->rec.type != SSL_RECORD_TYPE_APPLICATION_DATA || mode == HMAC_CREATE ||ssl->deBlockSize > 1 || (data == NULL && len == 0) )
#endif
	{
		psMd5Final(pmd5, ihash);

		psMd5Init(pmd5);
		psMd5Update(pmd5, key, MD5_HASH_SIZE);
		psMd5Update(pmd5, pad2, 48);
		psMd5Update(pmd5, ihash, MD5_HASH_SIZE);
		psMd5Final(pmd5, mac);
