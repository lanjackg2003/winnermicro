...
static void *ping_queue[PING_QUEUE_SIZE];
...
static u16 ping_test_chksum(unsigned short *addr,int len)
{
    int nleft=len;
    int sum=0;
...
static int ping_test_pack(int pack_no, char *sendpacket)
{
    ...
    *tval = tls_os_get_time();    /*记录发送时间*/
    ...
    icmp->chksum = ping_test_chksum((unsigned short *)icmp, packsize); /*校验算法*/
    return packsize;
}
...
static void ping_test_unpack(char *buf, int len, u32 tvrecv, struct sockaddr_in *from)
{
       ...
       (icmp->id == OWNER_PING_ID))
    {
        ...
        rtt = (tvrecv - (*tvsend)) * 10; /*以毫秒为单位计算rtt*/
        /*显示相关信息*/
        if (0 == rtt)
            printf("%d byte from %s: icmp_seq=%u ttl=%d rtt<1 ms\n",
...
static void ping_test_stat(void)
{
    ...
    printf("%u packets transmitted, %u received , %u(%.3g%%) lost.\n",
            send_cnt,  received_cnt, send_cnt - received_cnt, 
...
static void ping_test_send(int socket, struct sockaddr_in *dest_addr)
{
    ...
    if ((0 == g_ping_para.flags) && (MAX_NO_PACKETS == send_cnt))
        return;
...
static void ping_test_run(void)
{
    ...
    for ( ; ; )
    {
        ...
        if (!ping_test_abort)
        {
            tls_os_time_delay(g_ping_para.interval / 10);/* ms */
...
void ping_test_create_task(void)
{
    ...
    tls_os_queue_create(&ping_msg_queue, &ping_queue,
                        PING_QUEUE_SIZE, 0);
