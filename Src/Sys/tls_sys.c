...
#if TLS_CONFIG_AP
extern bool tls_ap_get_recflag(void);
#endif
...
#endif
#if TLS_CONFIG_APSTA
        case TLS_PARAM_IEEE80211_APSTA:
		{	
			struct tls_param_original_key origin_key;
			struct tls_param_bssid bssid;
			tls_param_get(TLS_PARAM_ID_BSSID, (void *)&bssid, true);
			tls_param_get(TLS_PARAM_ID_APSTA_SSID, (void *)&ssid2, true);
			if(bssid.bssid_enable)
			{
				tls_param_get(TLS_PARAM_ID_ORIGIN_KEY, (void *)&origin_key, true);
				if (0 == ssid2.ssid_len)
				{
                    tls_wifi_apsta_start_by_bssid(bssid.bssid, origin_key.psk, origin_key.key_length, NULL, 0);
				}
				else
				{
                    tls_wifi_apsta_start_by_bssid(bssid.bssid, origin_key.psk, origin_key.key_length, ssid2.ssid, ssid2.ssid_len);
				}
			}
			else
			{
				...
				if (0 == ssid2.ssid_len)
				{
                    tls_wifi_apsta_start(ssid.ssid, ssid.ssid_len, origin_key.psk, origin_key.key_length, NULL, 0);
				}
				else
				{
				    tls_wifi_apsta_start(ssid.ssid, ssid.ssid_len, origin_key.psk, origin_key.key_length, ssid2.ssid, ssid2.ssid_len);
                }
			}
			break;
		}
...
{
    tls_os_queue_send(msg_queue, 
            (void *)SYS_MSG_NET2_UP, 0);
}
static void tls_sys_net2_down(void)
{
    tls_os_queue_send(msg_queue, 
            (void *)SYS_MSG_NET2_DOWN, 0);
}
#endif
